...
from glue_utils import (
    convert_examples_to_seq_features,
...
def init_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Home directory of the trained ABSA model",
    )
    parser.add_argument(
        "--ckpt",
        type=str,
        required=True,
        help="Directory of model checkpoint for evaluation",
    )
    parser.add_argument(
        "--data_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="BIEOS",
        help="Tagging schema, should be kept same with " "that of ckpt",
    )
...
def predict(args, model, tokenizer):
    ...
    with open(f"{data_dir}/predictions.txt", "w") as text_file:
        #####
        for batch in tqdm(dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                for t in p_ts_sequence:
                    ...
                    ##### added by alex
                    output_ts_alex.append((aspect, sentiment))
                    #####
                print("Input: %s, output: %s" % (" ".join(words), "\t".join(output_ts)))
                ##### added by alex
                results.append(output_ts_alex)
                text_file.write(str(output_ts_alex) + "\n")
                #####
