...
def init_args():
    ...
    parser.add_argument(
        "--overfit", type=int, default=0, help="if evaluate overfit or not"
    )
...
def main():
    ...
    # Training
    if args.do_train:
        train_dataset, train_evaluate_label_ids = load_and_cache_examples(
    ...
    for checkpoint in checkpoints:
        ...
        if (
            ...
            or global_step == "train"
            or global_step == "fix"
            or global_step == "overfit"
        ):
            continue
        # validation set
    ...
    ) in zip(test_f1_values, test_loss_values, dev_f1_values, dev_loss_values):
        global_step = int(test_f1_k.split("_")[-1])
        if not args.overfit and global_step <= 1000:
            continue
        print(
            "test-%s: %.5lf, test-%s: %.5lf, dev-%s: %.5lf, dev-%s: %.5lf"
            % (
                test_f1_k,
                test_f1_v,
                test_loss_k,
